...
namespace JuegoInteractivo
{
    ...
    {
        // Nodo raíz que representa el inicio de la historia
        private Nodo _raiz;

        // Referencias al inventario y al personaje principal (el astronauta)
        private Inventario _inventario;
        private Personaje _astronauta;

        // Método público que devuelve el nodo raíz de la historia
        public Nodo ObtenerRaiz()
        {
            return _raiz;
        }

        // Constructor que inicializa la historia con el inventario y el astronauta
        public Historia(Inventario inventario, Personaje astronauta)
        {
            _inventario = inventario;
            _astronauta = astronauta;
            CrearHistoria(); // Crea y estructura la historia en forma de árbol de nodos
        }

        // Método privado que construye la historia, creando nodos y opciones
        private void CrearHistoria()
        {
            ...
            nodo2_2_1.Accion = () =>
            {
                ...
                if (clave == "1234")
                {
                    Console.WriteLine("La clave es correcta. La puerta se abre y entras en la estructura donde encuentras una nave espacial intacta, " +
                        "la cual usas para escapar. ¡Has sobrevivido!");
                ...
                else
                {
                    ...
                    Nodo finalBueno1 = new Nodo("Se desactiva el mecanismo de defensa. " +
                        "La puerta se abre y entras en la estructura donde encuentras una nave espacial intacta, la cual usas para escapar. ¡Has sobrevivido!");
            ...
            // Finales posibles
            Nodo finalBueno = new Nodo("Has encontrado el artefacto alienígena correcto y logras contactar con una estación espacial cercana. " +
                "Eres rescatado.");
        ...
        {
            // Inicializa el nodo actual como el nodo raíz de la historia
            Nodo nodoActual = _raiz;

            // Ciclo principal que recorre la historia mientras haya nodos
            while (nodoActual != null)
            {
                // Muestra la descripción del nodo actual (escena actual de la historia)
                Console.WriteLine(nodoActual.Descripcion);

                // Si el nodo actual tiene una acción asociada, se ejecuta
                if (nodoActual.Accion != null)
                {
                    nodoActual.Accion();
                }

                // Verifica si el nodo actual no tiene opciones (es un nodo final)
                if (nodoActual.OpcionIzquierda == null && nodoActual.OpcionDerecha == null)
                {
                    // Si no hay opciones, significa que la historia ha terminado
                ...
                // Lee la opción seleccionada por el jugador
                string opcion = Console.ReadLine();
                if (opcion == "1")
                {
                    // Si elige la opción 1, el nodo actual se actualiza a la opción izquierda
                    nodoActual = nodoActual.OpcionIzquierda;
                }
                else if (opcion == "2")
                {
                    // Si elige la opción 2, el nodo actual se actualiza a la opción derecha
